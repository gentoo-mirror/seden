...
IUSE="+accessibility cups dbus egl gles gif +glib gtkstyle mng nas nis opengl qt3support tiff trace xinerama"
...
src_configure() {
	...
	# OpenGL is set to an API or disabled completely
	if ! use opengl && ! use egl && ! use gles ; then
		myconf="${myconf} -no-opengl"
	fi
	if use opengl && ! use egl && ! use gles ; then
		myconf="${myconf} -opengl desktop"
	fi
	if use egl && ! use gles ; then
		epatch "${FILESDIR}/qt-gui-4.8.0-fix_GLESv1_libname.patch"
